...
def anuga_main_process(sim_name, **kwargs):
    ...
    if len(channel_pos) != channel_num or len(channel_state) != channel_num:
        print("BAD CONSTRUCTION!")
        return 0
    tag = simulation(sim_name, channel_num, channel_pos, branch_width, main_road_width, wall_height, gate_thickness, x_max, y_max, channel_state, debug=False)
    gate_pos = get_gate_locations(channel_num=channel_num, channel_pos=channel_pos, branch_width=branch_width, main_road_width=main_road_width, gate_thickness=gate_thickness, x_max=x_max, y_max=y_max)
    water_volums = get_water_volum(file_path, gate_pos)
    # print(water_volums)
    return water_volums
...
def simulation(sim_name, channel_num, channel_pos, branch_width, main_road_width, wall_height, gate_thickness, x_max, y_max, channel_state, debug=False):
...
def get_water_volum(sww_file, gate_poss):
